<template>
    <div class="flex flex-col ">
        <div class="flex flex-wrap gap-2 items-center text-xs md:text-sm px-5 md:px-10 py-2 bg-gray-100 text-gray-500">
            <router-link :to="{ name: 'home' }">Trang chủ </router-link>/<router-link
                :to="{ name: 'category', params: { id: product.category.categoryId } }"> {{ product.category.name
                }}</router-link>/<p class="line-clamp-1">{{ product.name }}</p>
        </div>
        <div class="flex flex-col gap-5 p-5 md:p-10 dark:bg-black dark:text-white">
                ...
                <div class="relative">
                    <div class="w-full flex flex-col gap-2 sticky top-1">
                        ...
                        <div class="flex flex-wrap gap-3">
                            <img v-for="image in product.productImages" @click="selectedImage = image.imageUrl"
                                :key="image.productImageId" :src="image.imageUrl" alt="" class="w-14 h-14 border-2">
                        </div>
                    </div>
                </div>
                    ...
                    <div class="flex flex-col">
                        <div class="flex justify-between border-b pb-5">
                            <div class="flex items-center">
                                <star-rating v-model="listComments.averageRate" :star-size="20" :show-rating="false"
                                    :increment="0.1" :read-only="true" :fixed-points="1" />
                                ({{ listComments.count }})
                            </div>
                        ...
                        <div class="flex items-center gap-3 py-2 text-center border-b">
                            <div v-if="discounted" class="w-16 py-1 font-bold bg-gray-100 text-red-500">-{{ discounted
                                }}%
                            </div>
                        ...
                        <div v-if="colors.length > 0"
                            class="flex items-center flex-wrap gap-3 py-2 text-center border-b text-sm">
                            <div v-for="item in colors" :key="item.productColorId"
                                @click="selectedColor = item.color.name"
                                :class="{ 'border-2 border-black': selectedColor === item.color.name }"
                                class="cursor-pointer w-10 h-10 rounded-full border flex justify-center items-center"
                                :style="{ backgroundColor: item.color.code }"></div>
                        </div>
                        <div v-if="sizes.length > 0"
                            class="flex items-center flex-wrap gap-3 py-2 text-center border-b text-sm">
                            <div v-for="item in sizes" :key="item.productSizeId"
                                @click="handleSelectedSize(item.size.name)"
                                :class="{ 'bg-black text-white': selectedSize === item.size.name }"
                                class="cursor-pointer w-20 h-10 border flex justify-center items-center">{{
                                    item.size.name }}</div>
                        </div>
                        <div class="flex flex-col gap-3">
                            <b class="flex gap-2 text-red-500">Bảo hành: <p class="text-black">2 năm</p></b>
                            <i><u><b>Lưu ý: </b></u>Sản phẩm hiện chỉ được cung cấp tại khu vực Hà Nội.</i>
                            <div class="w-full bg-blue-800 text-center text-xl uppercase py-2 text-white font-bold">Hãy
                                để MOho tư vấn cho bạn trước khi đặt hàng</div>
                            <p class="">Nhắn tin Fanpage: <a href=""><b class="text-white bg-blue-800 p-2">Nội thất
                                        moho</b></a></p>
                            <p class="">Gọi ngay Hotline: <b class=" text-blue-800 p-2">0918590630(Zalo)</b></p>
                            <div class="flex border w-32 h-8 font-bold text-xl text-center items-center">
                                <div @click="quantity > 0 && quantity--"
                                    class="bg-gray-100 w-8 cursor-pointer dark:bg-black dark:text-white">-</div>
                                <input type="text" v-model="quantity"
                                    class="w-16 text-center outline-none dark:bg-black dark:text-white" value="1">
                                <div @click="quantity < 10 && quantity++"
                                    class="bg-gray-100 w-8 cursor-pointer dark:bg-black dark:text-white">+</div>
                            </div>
                            <button @click="AddToCart"
                                class="font-bold text-white text-xl py-2 uppercase bg-blue-800">Thêm vào giỏ</button>
                            <button @click="BuyNow"
                                class="font-bold text-white text-xl py-2 uppercase bg-red-500 flex justify-center items-center">
                                Mua ngay
                            </button>
                            <p class="text-sm text-gray-500">(+) Miễn phí giao hàng & lắp đặt tại tất cả quận huyện
                                thuộc TP.HCM, Hà Nội, Khu đô thị Ecopark, Đại học Công Nghiệp Hà Nội (*)</p>
                            <p class="text-sm text-gray-500">(+) Miễn phí 1 đổi 1 - Bảo hành 2 năm - Bảo trì trọn đời
                                (**)</p>
                            <p class="text-sm text-gray-500">(*) Không áp dụng cho danh mục Đồ Trang Trí</p>
                            <p class="text-sm text-gray-500">(**) Không áp dụng cho các sản phẩm Clearance. Chỉ bảo hành
                                01 năm cho khung ghế, mâm và cần đối với Ghế Văn Phòng</p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="flex flex-col w-full border">
                <div class="w-full h-auto bg-gray-100 dark:bg-black dark:text-white border flex items-center">
                    <div @click="CurrentTab = 'tab1'" :class="{ 'bg-gray-300 dark:text-black ': CurrentTab === 'tab1' }"
                        class="p-2 hover:bg-gray-200 hover:dark:text-black cursor-pointer">Mô tả</div>
                    <div @click="CurrentTab = 'tab2'" :class="{ 'bg-gray-300 dark:text-black ': CurrentTab === 'tab2' }"
                        class="p-2 hover:bg-gray-200 hover:dark:text-black  cursor-pointer">Đánh giá ({{
                            listComments.count }})</div>
                    <div @click="CurrentTab = 'tab3'" :class="{ 'bg-gray-300 dark:text-black ': CurrentTab === 'tab3' }"
                        class="p-2 hover:bg-gray-200 hover:dark:text-black  cursor-pointer">Chính sách</div>
                </div>
                ...
                <div v-if="CurrentTab === 'tab2'" class="flex w-full p-2">
                    <div class="flex flex-col gap-2 w-full">
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <textarea v-model="commentText" class="border outline-none p-2"
                                placeholder="Viết gì đó ..." />
                            <div class="flex bottom-2 overflow-x-auto justify-between">
                                <div class=" flex flex-wrap">
                                    <label v-if="!commentImage" for="file"
                                        class="w-10 h-10 md:w-20 md:h-20 border-2 flex justify-center items-center hover:cursor-pointer">
                                        <VueIcon type="mdi" :path="mdiCamera" class="" />
                                    </label>
                                    <div v-else class="flex">
                                        <div class="relative">
                                            <img :src="commentImage" alt=""
                                                class="w-10 h-10 md:w-20 md:h-20 border-2 text-2xl block hover:cursor-pointer">
                                            <button type="button" @click="remove()">
                                                <VueIcon type="mdi" :path="mdiClose"
                                                    class="absolute top-0 right-0 p-1 hover:cursor-pointer border bg-slate-100 rounded-[50%]" />
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                <input type="file" hidden id="file" name="image" @change="handleImageChange">
                                <button @click="Comment" type="button"
                                    class="p-2 border bg-blue-500 text-white self-center rounded-sm">Send</button>
                            </div>
                        </div>
                        <div class=' mt-3 border-4 border-black dark:border-white  h-[105vh] p-2 relative'>
                                ...
                                <div v-for="item in listComments.comments" :key="item.commentId" class='border-b'>
                                    <div class='flex mt-3 items-center gap-3'>
                                        ...
                                        <div class=' flex flex-col'>
                                            ...
                                            <div class="flex">
                                                <VueIcon v-for="i in item.rate" :key="i" type="mdi" :path="mdiStar"
                                                    size="15" class=" text-yellow-300" />
                                            </div>
                            ...
                            <button class="w-full text-center bg-[#271511] text-white leading-8">Xem thêm bình luận
                                (10)</button>
                        </div>
                    </div>
                </div>
    ...
    methods: {
        ...
        async BuyNow() {
            try {
                ...
                this.$router.push({ name: "checkout", query: { productId: this.product.productId } });
            } catch (err) {
                this.$toast.error(`Đã xảy ra lỗi khi mua hàng`, {
                    position: "top-right",
                    timeout: 5000,
                });
                console.error(err);
